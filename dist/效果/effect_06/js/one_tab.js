    ...
    newVue({
        ...
        methods: {
            ...
            initDropLoad: function initDropLoad(wrapper) {
                var vm = this;
                // drop-load 须要容器具有两个必要条件：一是固定高度(wrapper.height())，二是DOM可见(wrapper[0].scrollHeight);
                // 所以出现问题一是查看是否固定了容器高度，二是查看容器DOM是否可见(包括其父元素隐藏导致的不可见);
                if (!wrapper.height()) {
                    console.error('[drop-load \u521D\u59CB\u5316\u9519\u8BEF]: \u6CA1\u6709\u7ED9 drop-load \u5BB9\u5668\u8BBE\u7F6E\u56FA\u5B9A\u9AD8\u5EA6');
                    return false;
                }
